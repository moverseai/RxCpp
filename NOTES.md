## Why
Some operators (_e.g._ `with_latest_from` were not compiling), and some recent performance changes (_i.e._ [PR562](https://github.com/ReactiveX/RxCpp/pull/562) and [PR564](https://github.com/ReactiveX/RxCpp/pull/564)) were not brought in from the `v4.1.1` release that the official `vcpkg` port uses. So we forked the `latest` version from `RxCpp` and brought in some extra changes that were needed.
